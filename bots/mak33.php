...
bot('sendMessage', [
'chat_id'=>$chat_id,
'text'=>"عذرا ⁉️ صديقي يجب عليك الاشتراك
بقناة البوت اولا • $ch • ثم اضغط /start",
]);return false;}
...
elseif($text == "الاوامر" && $chat_id == $admin){
sendaction($chat_id, typing);
        bot('sendmessage', [
                'chat_id' =>$chat_id,
                'text' =>"اهـلا عـزيـزي! انـت الـمـسـؤول هـنـا 😎",
                'parse_mode'=>'html',
      'reply_markup'=>json_encode([
            'keyboard'=>[
              [
              ['text'=>"🔢 عـدد الـمـشـتـركـيـن"],['text'=>"👥 رسـالـة لـلـكـل"],['text'=>"⤴️ ارسـال تـوجـيـة"]
              ]
              ],'resize_keyboard'=>true
        ])
            ]);
        }
elseif($text == "🔢 عـدد الـمـشـتـركـيـن" && $chat_id == $admin){
	sendaction($chat_id,'typing');
...
elseif($text == "👥 رسـالـة لـلـكـل" && $chat_id == $admin){
    file_put_contents("data/$from_id/ali_rahim","send");
	sendaction($chat_id,'typing');
	bot('sendmessage',[
    'chat_id'=>$chat_id,
    'text'=>"ارسـل نـص الـرسـالـة الـان",
    'parse_mode'=>'html',
    'reply_markup'=>json_encode([
      'keyboard'=>[
	  [['text'=>'الاوامر']],
...
elseif($ali == "send" && $chat_id == $admin){
	...
	bot('sendmessage',[
    'chat_id'=>$chat_id,
    'text'=>"تـم ارسـال لـلـجـمـيـع بـنـجـاح ✔️",
  ]);
...
elseif($text == "⤴️ ارسـال تـوجـيـة" && $chat_id == $admin){
    file_put_contents("data/$from_id/ali_rahim","fwd");
	sendaction($chat_id,'typing');
	bot('sendmessage',[
    'chat_id'=>$chat_id,
    'text'=>"ارسـل الـتـوجـيـة الـان ‼️",
    'parse_mode'=>'html',
    'reply_markup'=>json_encode([
      'keyboard'=>[
	  [['text'=>'الاوامر']],
...
Forward($fakar, $chat_id,$message_id); 
   ...
   bot('sendMessage',[ 
   'chat_id'=>$chat_id, 
   'text'=>"تـم اعـادة الـتـوجـيـة بـنـجـاح ✔️", 
   ]);
}
?>
