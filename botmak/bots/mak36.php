...
if (isset($update->message)) {
    ...
    if (isset($message->text)) {
        ...
        if ($text == "/start") {
            bot("sendMessage", [
                "chat_id" => $chat_id,
                "text" => "اهلا بك عزيزي العضو في بوت ترديد النصوص يمكنني التفاعل معك في ومع اعضاء مجموعتك",
                "reply_to_message_id" => $message_id,
                "parse_mode" => "Markdown",
            ]);
        } else {
            $t = file_get_contents("https://egy-jack.ml/api/api2.php?text=" . urlencode($text));
            $t = json_decode($t);
