...
$m = json_decode(file_get_contents("https://api-jack.ml/api6.php?Country=$text"),true);
$counrt = $m['الدوله'];
$city = $m['العاصمه'];
$macity = $m["مكان العاصمه"];
$temp = $m["درجه الحراره"];
$date = $m["التاريخ والوقت"];

if($text == '/start'){ 
bot('sendMessage',[
 'chat_id'=>$chat_id, 
  'text'=>" • اهلا عزيزي ( [$nx1$nx2](tg://user?id=$id) ) في معرفه معلومات الدول والعواصم
  ـ من خلال ارسال اسم الدوله باللغه الانجليزيه مثال `egypt`",
'parse_mode' =>"markdown",
'disable_web_page_preview' => true,
'reply_to_message_id'=>$message->message_id,]);}
